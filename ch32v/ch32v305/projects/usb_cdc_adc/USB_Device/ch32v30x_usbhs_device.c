/*
 * USBHS CDC Send — envio de datos del device al host
 *
 * Basado en el driver de eco (usb_cdc_echo), modificado para enviar
 * datos arbitrarios al host via EP2 IN (bulk).
 *
 * API:
 *   USBHS_CDC_Ready() — devuelve 1 si el USB esta enumerado y el TX libre
 *   USBHS_CDC_Send()  — copia data al buffer TX y arma EP2 IN
 */
 ...
 *          EP0: control, EP2 IN/OUT: bulk data, EP3 IN: notificacion CDC
 */
...
/*********************************************************************
 * @fn      USBHS_CDC_Ready
 * @brief   Devuelve 1 si USB esta enumerado y EP2 TX libre.
 */
uint8_t USBHS_CDC_Ready(void)
{
    return (USBHS_DevEnumStatus && !CDC_Tx_Busy);
}

/*********************************************************************
 * @fn      USBHS_CDC_Send
 * @brief   Envia datos al host por EP2 IN (bulk).
 *          data: puntero a los datos, len: longitud (max 512 bytes).
 */
void USBHS_CDC_Send(uint8_t *data, uint16_t len)
{
    if (!USBHS_DevEnumStatus || CDC_Tx_Busy) return;
    if (len > DEF_USB_EP2_HS_SIZE) len = DEF_USB_EP2_HS_SIZE;

    memcpy(USBHS_EP2_Tx_Buf, data, len);
...
/*********************************************************************
 * @fn      USBHS_IRQHandler
 * @brief   Interrupcion USBHS — maneja enumeracion, SETUP, y datos.
 */
    ...
    if (intflag & USBHS_UIF_TRANSFER)
    {
        switch (intst & USBHS_UIS_TOKEN_MASK)
        {
            ...
            case USBHS_UIS_TOKEN_IN:
                ...
                {
                    /* EP0 IN — envio de descriptores, etc. */
                    ...
                    /* EP2 IN completo — transmision terminada */
                    ...
                    /* EP3 IN (notificacion CDC, no usado) */
            ...
            case USBHS_UIS_TOKEN_OUT:
                switch (intst & (USBHS_UIS_TOKEN_MASK | USBHS_UIS_ENDP_MASK))
                {
                    ...
                    /* EP2 OUT — datos del host, descartar y rearmar */
